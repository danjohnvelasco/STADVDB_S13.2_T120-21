...
router.get('/query4', (req, res) => {
    ...
    conn.query('SELECT c.customer_city AS "City", QUARTER(o.order_purchase_timestamp) AS "Quarter Of Purchase", p.product_category_name AS "Product Category", sum(s.unit_sales) AS "Unit Sales" FROM customers c INNER JOIN sales s ON c.customer_id = s.customer_id INNER JOIN products p ON s.product_id = p.product_id INNER JOIN orders o ON s.order_id = o.order_id WHERE YEAR(o.order_purchase_timestamp) = 2018 AND (c.customer_city = "sao paulo"  OR c.customer_city = "abaetetuba") AND (p.product_category_name  = "fashio_female_clothing" OR p.product_category_name = "fashion_male_clothing") AND (QUARTER(o.order_purchase_timestamp) = 1 OR QUARTER(o.order_purchase_timestamp) = 2) GROUP BY c.customer_city, QUARTER(o.order_purchase_timestamp), p.product_category_name WITH ROLLUP;',
    function (err, results, fields) {
        if (err) {
            console.error(err);
            res.json([{id:1, error: "Query failed. Check your inputs."}]);
