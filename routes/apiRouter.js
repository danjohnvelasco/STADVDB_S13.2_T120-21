...
router.get('/query1', (req, res) => {
    ...
    conn.query('SELECT title, genre, year FROM movies WHERE genre LIKE ? AND year BETWEEN ? AND ? ORDER BY year LIMIT 2', [genre,parseInt(data.startYear),parseInt(data.endYear)], 
...
router.get('/query2', (req, res) => {
    ...
    conn.query('SELECT title, country, language, year FROM movies WHERE language LIKE ? AND country LIKE ? AND year BETWEEN ? AND ? ORDER BY year LIMIT 2', [language,country,parseInt(data.startYear),parseInt(data.endYear)], 
        function (err, results, fields) {
            if (err) throw err;
            else console.log('Selected ' + results.length + ' row(s).');
...
router.get('/query4', (req, res) => {
    ...
    console.log(req.query.data);
    var data = req.query.data;
    conn.query('SELECT DISTINCT m.title, r.weighted_average_vote AS rating FROM movies m CROSS JOIN ratings r ON m.imdb_title_id = r.imdb_title_id WHERE m.year = ? AND r.weighted_average_vote BETWEEN ? AND ? ORDER BY rating DESC', [parseInt(data.year),parseInt(data.min_avg_vote),parseInt(data.max_avg_vote)], 
        function (err, results, fields) {
            if (err) throw err;
            else console.log('Selected ' + results.length + ' row(s).');
